...
for(i in 1:1000){print(i)
  ...
  for(k in 1:5){
    ...
    mtx<-table(true=factor(test$survival,levels=0:1),pred=factor(pred,levels=0:1))
    ...
    auprc[k]<-pr.curve(scores.class0=predict(model,test,type="prob")[,2][test$survival==1],
                       scores.class1=predict(model,test,type="prob")[,2][test$survival==0])$auc.integral
  }
  acc.mean[i]<-mean(acc)
...
saveRDS(results.bootstrap,"inner.results.bootstrap.rf.RDS")